...
- **逐屏原生截取：省像素，也省时间**（协议 v4 的 `ScreenshotArea`，见 docs/capture-linux.md §3.3）。
  整屏舞台图的尺寸是"逻辑并集 × 桌面最大缩放"，本机双屏是 6720x2412 = 16.2 Mpx；
  按每块屏自己的矩形分别拍则是 2560x1600 + 3840x2160 = 12.4 Mpx，**少 24%**，而且每块屏拿到的
  是原生像素、不含插值。实测（dev 构建）整屏那条路：`Screenshot` D-Bus 往返 900.5 ms、
  我方解 6720x2412 的 PNG 114.5 ms。单张编码的参考成本：2880x1800 约 284 ms、3840x2160 约 299 ms，
  所以并行发请求（每次一个新的 `Shell.Screenshot` 实例，shell 侧编码在工作线程里重叠）
  比串行拍两张重要得多。**要现场量就跑**
  `cargo test --lib capture_stage_timings -- --ignored --nocapture`：它先逐屏拍一遍打印每块屏的
  尺寸与耗时，再打印整屏那条路的分段，v3/v4 的对比一条命令出。注意抬过协议版本后必须
  **注销重登一次**逐屏才生效。
