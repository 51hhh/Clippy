...
| 录屏 / 音频 / 编码 | 20 | 🟡 | ❌ | 无音频和产品入口；恢复清单、有界时间线、X11/WGC/AVFoundation 帧源骨架及 VP9 原型已建立 |
...
继续生产和控制。默认编码器、平台原生验收和产品入口仍未完成，不能计作可用录屏。MJPEG/AVI
诊断分段已实现流式写入、固定帧率补帧、
...
领域生命周期；可信 Tauri 开始 IPC 和产品编码策略仍未开放，不能据此把 VP9 作为默认。统一分段
...
提交期间同一调用者/请求槽不能被新请求替换。截图、图片 OCR/扫码/保存/Pin 和翻译适配器仍未接入。
