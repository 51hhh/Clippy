...
| 长截图 | 15 | 🟡 | 🟡 | 固定选区、上下左右手动追加、回访保护、前接与撤销已实现；无自动滚动和输入透明 guide |
| 标注与图像效果 | 23 | 🟡 | 🟡 | 16 种工具及模糊、马赛克、聚光灯、放大镜、调色和圆角已接入；无语义智能擦除 |
| 贴图 / 历史 / 分组 | 29 | 🟡 | 🟡 | 临时 Pin 与用户保存的工作区已分离，支持布局恢复和分组数据；分组管理界面与完整历史浏览仍待实现 |
...
代码中没有滚轮或平台输入注入驱动自动滚动；输入透明 guide 和四平台真实透传验收仍未完成。
...
- 缩放、透明度、锁定、置顶和拖动由单个 180 ms 合并 worker 写回；窗口刚映射时若暂时读不到位置，
  只更新其它状态并保留最后可信位置；
- 工具条与右键菜单提供保存/移除工作区入口。分组的创建、重命名、删除、分配协议和外键语义已完成，
  可视分组管理器及工作区历史浏览仍留在本需求的后续界面阶段；
...
工作区执行 `./scripts/ci-local.sh`：19 个步骤通过、0 失败、2 个可选项跳过。其中 Rust 831 项通过、
12 项忽略；X11 私有协议 4/4 通过；前端 64 个文件、1163 项测试通过；DOM、Canvas 与布局像素
smoke、TypeScript、Vite 生产构建均通过。非宿主平台交叉 lint 与 AppImage 可视 smoke 未启用，
不计为通过；当前提交尚未在同一 SHA 上运行 Windows/macOS 原生 CI，也没有完成 Pin 工作区混合
DPI/显示器热插拔真机验收。参考软件、模型精度基准和录屏技术原型不属于这轮实现验证。
