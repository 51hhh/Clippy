...
| 录屏 / 音频 / 编码 | 20 | 🟡 | ❌ | 无音频；原生 X11 有受门控入口，恢复库、按需 WebM 播放、异常分段无损 remux、三平台帧源与 VP9 原型已建立，仍待真机验收 |
...
| P2（X11 入口、结果库、播放与恢复 remux 已完成，待真机/音频） | `PX-REC-01` / `PX-REC-PLAYBACK-01` / `PX-REC-MERGE-01` | 可恢复录屏最小闭环 | 平台采集/编码实测 |
