...
| 录屏 / 音频 / 编码 | 20 | 🟡 | ❌ | 无音频；原生 X11 有受门控入口，恢复库、按需 WebM 播放、三平台帧源与 VP9 原型已建立，仍待真机验收 |
| 动作 / 脚本 / 启动器 | 12 | 🟡 | 🟡 | 七个类型化动作、权限模型、键盘启动器与安全组合已交付；任意脚本运行时明确不在首阶段范围 |

原表的整体方向成立。当前校正重点包括：增强 OCR 是“实现完成、交付未完成”；扫码已完成四种
产品格式；录屏已有 X11 受门控入口但尚未默认交付；类型化动作与启动器已完成，任意脚本运行时不在
首阶段范围。
...
控制窗和结果/恢复库。结果窗可在完整校验后用不透明租约按需播放 WebM，不把真实路径交给前端。
产品入口仍只在显式 feature 的原生 X11 开放，系统音频、麦克风、恢复分段合并和各平台真机矩阵尚未完成。
...
| P2（X11 受门控入口、结果库与按需播放已完成，待真机/音频） | `PX-REC-01` / `PX-REC-PLAYBACK-01` | 可恢复录屏最小闭环 | 平台采集/编码实测 |
...
feature + 原生 X11 的受门控区域选择入口与结果/恢复库。默认构建、其他平台入口、真机验收和音频
仍保持未完成。
...
控制 IPC 已接入；后续已将可信开始入口接到显式 VP9 feature 的原生 X11 录屏覆盖层，普通截图会话
仍不能升级为录屏。
...
领域生命周期；可信 Tauri 开始 IPC 已接到显式 VP9 feature 的原生 X11 入口，默认产品编码策略仍未
