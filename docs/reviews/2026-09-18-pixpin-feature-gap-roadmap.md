...
大小与 SHA-256 校验、符号链接拒绝、原子状态回写和损坏尾段截断均有单元测试。当前只恢复未来录制
会话的数据合同。单调采集时间、暂停区间扣除、固定几何与紧凑 RGBA 合同、64 MiB 单帧上限和三槽
有界队列也已由合成帧测试固定；队列满时保留待编码头帧与最新画面并记录背压丢帧。编码器、平台
采集和产品入口仍未实现，不能计作可用录屏。MJPEG/AVI 诊断分段已实现流式写入、固定帧率补帧、
...
1–120 fps 节流、停止唤醒、错误回传和强制 join；暂停/继续使用帧源同一单调时钟，暂停期间不取帧。
产品会话/控制窗接线、编码消费、控制窗排除与真机性能验收仍未完成。
