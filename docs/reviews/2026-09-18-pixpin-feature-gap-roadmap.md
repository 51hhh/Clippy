...
| 录屏 / 音频 / 编码 | 20 | 🟡 | ❌ | 可恢复 VP9/Opus 双轨 session 与异常恢复已完成；Windows/Linux/macOS QA 已接系统声、麦克风、固定增益双源混音与开始前设备选择，macOS 麦克风支持 8–192 kHz→48 kHz sinc 重采样；四平台真机验收仍待闭环 |
| 动作 / 脚本 / 启动器 | 12 | 🟡 | 🟡 | 七个类型化动作、权限模型、键盘启动器与安全组合已交付；任意脚本运行时明确不在首阶段范围 |

原表的整体方向成立。当前校正重点包括：增强 OCR 是“实现完成、交付未完成”；扫码已完成四种
产品格式；录屏已有 X11/Wayland/Windows/macOS 受门控 QA 入口但尚未默认交付；类型化动作与
启动器已完成，任意脚本运行时不在首阶段范围。
...
feature 的原生 X11 与 Windows/macOS/Wayland QA 构建开放，默认 release 保持关闭。双轨
...
VP9 packet，并按每段 Opus pre-skip/尾 padding 还原真实 PCM 后连续重编码音轨。macOS QA 的
非 48 kHz 麦克风已由固定 sinc 链归一化为 48 kHz；三平台 QA 构建可把系统声和默认麦克风按共享
48 kHz sample grid、100 ms 有界迟到窗口和固定 −6 dB/路混为现有单一 Opus 音轨。QA 构建还会
枚举当前设备，通过 caller 绑定的一次性不透明目录选择非默认源，并把原生身份固定进本次 source
plan。录制中设备热切换和各平台真机矩阵尚未完成。
...
| P2（Windows/Linux/macOS QA 接线、双轨缩略图与异常恢复已完成，待原生真机） | `PX-REC-CLOCK-01` / `PX-REC-AUDIO-01` / `PX-REC-AUDIO-WORKER-01` / `PX-REC-WINDOWS-AUDIO-01` / `PX-REC-AV-EPOCH-01` / `PX-REC-OPUS-WEBM-01` / `PX-REC-AV-MANIFEST-01` / `PX-REC-AV-SESSION-01` / `PX-REC-WINDOWS-AV-QA-01` / `PX-REC-MACOS-AUDIO-01` / `PX-REC-MACOS-MIC-01` / `PX-REC-MACOS-RESAMPLE-01` / `PX-REC-LINUX-AUDIO-01` / `PX-REC-AUDIO-MIX-01` / `PX-REC-AUDIO-DEVICE-01` / `PX-REC-AV-THUMBNAIL-01` / `PX-REC-AV-MERGE-01` | 单一会话时钟、48 kHz PCM、显式 A/V 起点、有界双轨排序、参考 Opus、周期可恢复双轨容器、严格双轨首帧缩略图、异常双轨恢复，以及 Windows/Linux/macOS 音源、固定重采样、有界混音与非默认设备选择的受门控产品接线 | `PX-REC-01` 视频时间线 |
...
行为不变；异常双轨分段的合并入口已由后续 `PX-REC-AV-MERGE-01` 交付。完整合同见
[`2026-09-22-recording-av-thumbnail.md`](../superpowers/specs/2026-09-22-recording-av-thumbnail.md)。

**2026-09-22 双轨异常恢复进度**：`PX-REC-AV-MERGE-01` 为 schema v2 增加严格双轨恢复。
...
**2026-09-22 macOS 系统音频 QA 进度**：`PX-REC-MACOS-AUDIO-01` 新增非默认
`recording-macos-av-qa` 组合 feature，在 macOS 13+ 把冻结选区所属显示器的 ScreenCaptureKit 系统声送入现有
...
**2026-09-23 macOS 麦克风重采样进度**：`PX-REC-MACOS-RESAMPLE-01` 让 ScreenCaptureKit 回调
保留设备 8–192 kHz 整数原生率，只在音频 worker 内用固定 Rubato asynchronous sinc 转成 48 kHz
stereo；48 kHz 输入逐 sample 直通。原生 PTS 按输入速率计算，连续 packet 共用滤波状态，空洞切段，
正常 Stop 提交精确尾帧，Pause/Drop 丢弃旧状态。依赖版本、Cargo checksum 与 MIT/Apache-2.0
许可证资源已进入供应链门禁；44.1/48/88.2/96 kHz 内建/外接设备、短录音首尾和 30 分钟漂移仍须
macOS Intel/Apple Silicon 同一 SHA 真机记录。完整合同见
[`2026-09-23-recording-macos-resampler.md`](../superpowers/specs/2026-09-23-recording-macos-resampler.md)。

**2026-09-23 双源混音进度**：`PX-REC-AUDIO-MIX-01` 在现有音频 worker 内同时持有系统声与默认
麦克风 source，并用同一个 session clock 映射到 48 kHz sample grid。每路最多等待 100 ms packet
抖动，随后以确认过的静音补洞；迟到越过 watermark、重叠、格式变化或一秒 staging 超限都会中止，
不静默丢音。两路固定各乘 0.5 后相加为 stereo，20 ms 输出继续进入现有 Opus/WebM、恢复和缩略图
链。Pause 清空未提交混音状态，Stop 先取得两路有限尾块；平台统一包装器同时补齐了单音源
`take_stopped_chunks` 转发，避免 macOS sinc Stop 尾帧在产品接线中丢失。Windows/Linux QA 始终在
系统声与默认麦克风同时可用时声明新模式，macOS 只在 15+ 同时声明；默认/release 仍关闭。完整合同见
[`2026-09-23-recording-audio-mix.md`](../superpowers/specs/2026-09-23-recording-audio-mix.md)。

**2026-09-23 音频设备选择进度**：`PX-REC-AUDIO-DEVICE-01` 在开始录制前异步枚举当前设备：
Windows 使用活动 WASAPI render/capture endpoint，Linux 使用 PipeWire `Audio/Sink` 与非 monitor
`Audio/Source`，macOS 15+ 使用 AVFoundation 麦克风发现；ScreenCaptureKit 系统声仍保持系统默认。
Rust 为每个 Recording 覆盖层签发一次性、有过期时间且全局有界的目录，前端只得到清洗后的标签、
默认标记和不透明 token。显式选择分别进入 `GetDevice`、PipeWire `target.object` 与
`microphoneCaptureDeviceID`，设备消失会让本次启动失败，不会静默回退。三平台原生设备、同名项、
开始前/录制中拔出和 30 分钟漂移仍须同一 SHA 真机记录；完整合同见
...
系统声/默认麦克风适配器。stream 只协商 48 kHz stereo F32LE FL/FR，并在格式确认后请求
`SPA_META_Header`；原生 PTS 通过 `CLOCK_MONOTONIC` 中点校准映射到 session clock，gap 生成等长
静音，普通 packet 拆成最多 20 ms 的公共 PCM chunk。PipeWire 对象留在独立 loop 线程，八块有界
桥接满、时间线倒退、格式或 buffer 结构异常都会中止两轨，暂停/继续通过 stream active 状态并过滤
迟到块。Ubuntu 原型 CI 与 Linux Native QA 包会显式编译该 feature，默认/release 保持关闭；GNOME
X11/Wayland、KDE、wlroots 的真实默认设备、服务重启、强杀恢复和 30 分钟漂移仍待同一 SHA 记录。
完整合同见
[`2026-09-22-recording-linux-audio.md`](../superpowers/specs/2026-09-22-recording-linux-audio.md)。
...
15+ 默认麦克风也已接通；macOS 麦克风固定 sinc 重采样、三平台双源固定增益混音与开始前非默认
设备选择已完成代码合同。默认构建、录制中设备热切换与真机验收仍未完成。
