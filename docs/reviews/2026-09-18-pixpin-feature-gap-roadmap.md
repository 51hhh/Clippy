...
| 长截图 | 15 | 🟡 | 🟡 | 固定选区、上下左右手动追加、回访保护、前接、撤销及 X11 自动滚动已实现；无 Wayland/Windows/macOS 自动输入和透明 guide |
...
展示自动入口。输入透明 guide、X11 自动链真机证据和四平台真实透传验收仍未完成。
...
字节合计受 256 MiB 上限约束。普通截图覆盖层在 handoff 后已退出，当前可直接滚动原窗口；输入透明
guide 尚未实现，不能把最后一项平台验收记为通过。自动滚动仍留在 `PX-LS-AUTO-01`。
...
调用被测库的编码器。坏 PNG、CRC、尺寸/像素/字节上限和输出预算沿用同一模块既有回归。
