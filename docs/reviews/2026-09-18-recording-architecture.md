...
触发追赶式突发采集；停止通过条件变量立即唤醒，线程错误和 pipeline 错误会交给 owner，`Drop` 也会
停止并 join，禁止会话结束后留下桌面读取线程。worker 只写三槽 pipeline，不接触编码、文件或 UI。
暂停/继续控制、编码消费线程、控制窗排除和 X11 真机性能证据仍未接入，因此还不能从 UI 开始录屏。
