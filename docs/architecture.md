...
| `recording/` | `RecordingLifecycle` 串起可信选区、平台帧源、三槽背压、共享单调时钟、VP9/WebM writer、恢复分段、控制窗和结果库；内部音频层已固定 48 kHz 单音轨 pipeline 与线程内 source worker，但尚未接平台音源、Opus、session 或 WebM 音轨。默认产品入口仍只在显式 feature 的原生平台 QA 构建开放。`manifest.rs` 拥有原子 journal、长度/SHA-256 与删除/导出安全合同；`media_protocol.rs` 在完整校验后签发结果窗专属租约，以最大 2 MiB 的单 Range 响应支持 WebM 库内播放，WebView 不接触真实路径。 |
