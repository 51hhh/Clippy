...
| `pin/project.rs` | 可编辑 PNG v3 的信任边界。标准 PNG 的 IDAT 是最新合成图，真正压缩的 `clippy-project` iTXt 自包含 canonical 原图、尺寸/sha256、annotations、adjustments 和 rendererVersion；`preview.rgbaSha256` 再以“宽高 + 解码 RGBA”绑定当前 IDAT，拒绝把合法工程块移植到另一张图，同时允许同像素 PNG 改变压缩级别或 chunk 切分。v2 仍可读取，并在下一次可编辑保存时升级为 v3。运行时 `PinSource::Project` 分开保存 source 与 flattened preview：屏幕/快速复制走 preview，画布/再次保存按需取 source。writer 与 reader 共用 64 MiB 合成图、64 MiB 原图、96 MiB 解压 JSON、160 MiB 容器预算；v1、未来版本、损坏元数据或摘要不匹配只丢可编辑性，IDAT 仍按普通 PNG 打开。editable 保存失败不得静默伪装成 flat；flat 导出会重编码并移除工程块，避免模糊/马赛克文件泄露内嵌原图 |
