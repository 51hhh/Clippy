...
| `recording/` | `RecordingLifecycle` 串起可信选区、平台帧源、有界背压、共享单调时钟、VP9/WebM writer、恢复分段、控制窗和结果库；`RecordingManager` 在同一 generation 槽拥有纯视频或 VP9 + Opus 双轨 session。内部音频固定为 48 kHz mono/stereo，Windows AV QA feature 已把 WGC 与 WASAPI 系统声/默认麦克风接线，模式由后端能力下发且默认无音频；macOS/Linux 音源与正式发布仍未接入。`manifest.rs` 拥有原子 journal、长度/SHA-256 与删除/导出安全合同；`media_protocol.rs` 在完整校验后签发结果窗专属租约，以最大 2 MiB 的单 Range 响应支持 WebM 库内播放，WebView 不接触真实路径。 |
