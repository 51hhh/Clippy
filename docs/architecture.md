...
| `react/pin/` | 首帧就绪、工具栏、拖动阈值和 rAF 更新合并；`gestures.ts` 是"不可缩放、不可划选"的纯规则（滚轮缩放、Shift+滚轮调不透明度、ctrl/meta+滚轮一律忽略），由 `App.tsx` 里 `{ passive: false }` 的原生监听器执行——React 的 `onWheel` 是被动监听器，在里面 `preventDefault()` 拦不住 WebKit 的页面缩放。拖动判据只看 `event.buttons`（`pointerStillHeld`）：Wayland 上 `startDragging` 之后指针被合成器抓走，迟到的 `pointercancel` 会落在下一次 `pointerdown` 之后，任何跨事件记账都会被它抹掉（症状是"隔一次拖不动"）。`update_pin` 同一时刻只许一个请求在飞、在飞就攒着——它是同步命令、跑在 GTK 主线程且带一次 D-Bus 摆位，按 rAF 无条件发就是每秒 60 次往主线程压活 |
| `react/pin/rendering.ts` | 贴图该用哪种 `image-rendering`。GTK3 不支持 `wp_fractional_scale_v1`，1.5 倍缩放的桌面上 WebKit 只拿到**整数缓冲区缩放 2**，按原物理尺寸显示就要先被平滑放大 4/3 再被合成器缩回 3/4，头一趟就把细节抹平了——这是"选区里清楚、贴出来糊"的根因。最近邻放大 4/3 再按 3/4 平均缩回几乎互为逆运算，实测（成像与源图逐像素比 PSNR）默认 30.06 dB → `pixelated` 33.73 dB；但缩到 0.6 倍时反过来差 4.5 dB，所以判据是 **显示尺寸 × 真实缩放 == 图片像素数**，而不是 `scale == 1`（全屏贴图会被 `origin_content_size` 缩小而 `scale` 仍是 1）。真实缩放由后端 `desktop_scale_at` 查出来随 payload 下发，`devicePixelRatio` 顶替不了（它就是那个整数 2） |
