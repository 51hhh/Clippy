...
| `react/pin/` | 首帧就绪、工具栏、拖动阈值和 rAF 更新合并；`gestures.ts` 是"不可缩放、不可划选"的纯规则（滚轮缩放、Shift+滚轮调不透明度、ctrl/meta+滚轮一律忽略），由 `App.tsx` 里 `{ passive: false }` 的原生监听器执行——React 的 `onWheel` 是被动监听器，在里面 `preventDefault()` 拦不住 WebKit 的页面缩放。拖动判据**只看每个事件自身**（`trackDragPointerDown`/`trackDragMove`）：Wayland 上 `startDragging` 之后指针被合成器抓走，这一次的 `pointerup` 不再送到 WebKit，于是每次拖完都留下一件迟到的事情落在**下一次**按压之后——迟到的 `pointercancel`、`buttons=0` 的收尾 `pointermove`，或者 WebKit 那份"按键还按着"的残留把下一个 `pointerdown` 吃掉。三种都表现为"隔一次拖不动"，也都躲不过，因为被污染的正是跨事件的记账本身。所以起点可以从 `pointermove` 自己长出来、不必等 `pointerdown`，松手清空一切（含工具条抑制），300 ms 限速只防"合成器没接手指针"那一路且被任何新按压清零。`update_pin` 同一时刻只许一个请求在飞、在飞就攒着——它是同步命令、跑在 GTK 主线程且带一次 D-Bus 摆位，按 rAF 无条件发就是每秒 60 次往主线程压活 |
