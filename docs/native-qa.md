...
录屏原型当前只在同一 SHA 的 Linux X11、Windows 10/11 与 macOS 12.3+ QA 包开放；没有音频。每次先保存安装包
...
5. Wayland 四个 profile 执行 `recording_entry_gated`：入口不得显示，直接请求开始仍须拒绝。普通
   macOS 11 release 包也保持门控；不要通过开发者控制台或修改 feature 绕过门控。

Linux X11、Windows 与 macOS 的三项录屏记录全部完成前，对应 `PX-REC-WINDOWS-QA-01` 与
`PX-REC-MACOS-SCK-01` 保持待验收；编译、Xvfb、GitHub runner 或合成帧不能替代原生桌面证据。
