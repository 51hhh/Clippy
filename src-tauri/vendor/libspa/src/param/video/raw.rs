...
impl VideoInfoRaw {
    pub fn new() -> Self {
        // Ubuntu 22 的 PipeWire 0.3.48 头文件还没有 `flags` 字段，而且 `modifier`
        // 仍是有符号整数。上游用字段字面量构造 bindgen 类型，导致 crate 在 Jammy 上
        // 编译失败。这个 C 结构只由整数和两个整数 POD 组成；其官方
        // SPA_VIDEO_INFO_RAW_INIT 宏也是全零初始化，随后 parse() 再写入协商结果。
        // SAFETY: `spa_video_info_raw` 不含引用或必须非零的枚举/指针，全零是 C API 定义
        // 的合法初态，并且同时适用于 0.3.48 与加入 flags 后的新结构布局。
