...
// 先读取属性元信息，不能在取回完整不可信大属性后才检查内存预算。
fn get_property_bounded(
	connection: &RustConnection,
	window: u32,
	property: u32,
	target: u32,
	budget: usize,
) -> Result<x11rb::protocol::xproto::GetPropertyReply> {
	let probe = connection
		.get_property(false, window, property, target, 0, 0)
		.map_err(into_unknown)?
		.reply()
		.map_err(into_unknown)?;
	if probe.type_ != target {
		return Ok(probe);
	}
	if probe.bytes_after as usize > budget {
		return Err(Error::ConversionFailure);
	}
	let length = (budget / 4 + usize::from(budget % 4 != 0)).min(u32::MAX as usize) as u32;
	let reply = connection
		.get_property(true, window, property, target, 0, length)
		.map_err(into_unknown)?
		.reply()
		.map_err(into_unknown)?;
	// probe 和读取之间对方可能改属性；仍需校验，不接受被截断的数据。
	if reply.bytes_after != 0 || reply.value.len() > budget {
		return Err(Error::ConversionFailure);
	}
	Ok(reply)
}
...
impl ManagerHandoverState {
	fn active(&self) -> bool {
		matches!(self, Self::InProgress | Self::Transferring)
	}
}
...
#[derive(Default)]
struct Selection {
	data: RwLock<Option<Arc<Vec<ClipboardData>>>>,
...
impl Inner {
	...
	fn write(
		...
		*data_guard = Some(Arc::new(data));
		...
		self.server
			...
			// Clippy: 等待实际写连接完成 ownership 更新，避免 watcher 先读到新数据、
			// 下一轮才收到本次 SelectionNotify，误把内部复制当成新的外部复制。
			.check()
			.map_err(into_unknown)?;
	...
	fn read_single(
		...
		let total_deadline = Instant::now() + incr::TOTAL_TIMEOUT;

		while Instant::now() < timeout_end.min(total_deadline) {
				...
				Event::SelectionNotify(event) => {
					...
					match result {
						...
						ReadSelNotifyResult::IncrStarted => {
							...
							// reset our timeout.
							timeout_end = Instant::now() + incr::IDLE_TIMEOUT;
						}
	...
	fn handle_read_selection_notify(
		...
		let mut reply = get_property_bounded(
			&reader.conn,
			event.requestor,
			event.property,
			event.target,
		...
		if reply.type_ == target_format {
			...
			*using_incr = true;
			let minimum = reply.value32().and_then(|mut values| values.next());
			if reply.format != 32
				|| reply.value_len != 1
				|| minimum.map_or(true, |length| length as usize > MAX_RECEIVE_BYTES)
			{
				return Err(Error::ConversionFailure);
			}
			// 提示值不可信，不按它一次性 reserve；实际分块也逐次检查总预算。
			let _ = incr_data;
	...
	fn handle_read_property_notify(
		...
		let remaining = MAX_RECEIVE_BYTES.saturating_sub(incr_data.len());
		let reply =
			get_property_bounded(&reader.conn, event.window, event.atom, target_format, remaining)?;

		// log::trace!("Received segment. value_len {}", reply.value_len,);
		if reply.type_ != target_format
			|| reply.format != 8
			|| incr_data.len().saturating_add(reply.value.len()) > MAX_RECEIVE_BYTES
		{
			return Err(Error::ConversionFailure);
		}
		...
		// 合法分块可受桌面调度影响超过 10ms；同时由 read_single 的总截止防止无限滴流。
		*timeout_end = Instant::now() + incr::IDLE_TIMEOUT;
	...
	fn handle_selection_request(
		&self,
		mut event: SelectionRequestEvent,
		sender: &mut incr::Sender,
	) -> Result<bool> {
		...
		if sender.property_in_use(event.requestor, event.property) {
			// 同 key 的小文本/TARGETS 也不能覆盖仍在传输的旧 PNG 属性。
			success = false;
			complete = false;
			...
			let data = self.selection_of(selection).data.read().clone();
			if let Some(data_list) = data {
				success = match data_list.iter().position(|d| d.format == event.target) {
					Some(index)
						if data_list[index].bytes.len()
							> incr::direct_payload_limit(
								self.server.conn.maximum_request_bytes(),
							) =>
					{
						match sender.begin(self, event, Arc::clone(&data_list), index) {
							Ok(()) => {
								complete = false;
								true
							}
							Err(error) => {
								warn!("Cannot begin INCR transfer: {error}");
								false
							}
						}
					}
	...
	fn ask_clipboard_manager_to_request_our_data(&self) -> Result<()> {
		...
		// 没有 clipboard manager 时不等待；有 manager 时给 INCR 有界传输窗口。
		if self
			.server
			.conn
		...
		*handover_state = ManagerHandoverState::InProgress;
		...
		loop {
			let timeout = match *handover_state {
				ManagerHandoverState::Finished => return Ok(()),
				ManagerHandoverState::Failed => {
					return Err(Error::unknown("Clipboard manager transfer failed"))
				}
				ManagerHandoverState::Transferring => incr::TOTAL_TIMEOUT,
				_ => LONG_TIMEOUT_DUR,
			};
			let remaining = (started + timeout).saturating_duration_since(Instant::now());
			if remaining.is_zero() {
				return Err(Error::unknown("Clipboard manager handover timed out"));
			}
			self.handover_cv.wait_for(&mut handover_state, remaining);
		}
	}
}
...
fn serve_requests(context: Arc<Inner>) -> Result<(), Box<dyn std::error::Error>> {
	...
	let mut sender = incr::Sender::default();
	let mut written = false;
	let mut notified = false;
	let mut handover_started = false;
	loop {
		let failed = sender.expire(&context, Instant::now());
		{
			let mut state = context.handover_state.lock();
			if state.active() {
				if failed {
					*state = ManagerHandoverState::Failed;
					context.handover_cv.notify_all();
				} else if written && notified && sender.is_empty() {
					*state = ManagerHandoverState::Finished;
					context.handover_cv.notify_all();
				}
			}
		}
		let event = if sender.is_empty() {
		...
		if !handover_started && context.handover_state.lock().active() {
			// 退出前的普通请求/过期不能充当此次 handover 的完成或失败。
			handover_started = true;
			written = false;
			notified = false;
		}
		match event {
			Event::DestroyNotify(event) if event.window == context.server.win_id => {
				sender.abort_all(&context);
				return Ok(());
			}
			Event::DestroyNotify(event) => {
				if sender.destroy_requestor(&context, event.window) {
					*context.handover_state.lock() = ManagerHandoverState::Failed;
					context.handover_cv.notify_all();
				}
			}
			Event::PropertyNotify(event) => {
				if let Some((completed, for_handover)) = sender.acknowledge(&context, event) {
					if for_handover {
						written |= completed;
						if !completed {
							*context.handover_state.lock() = ManagerHandoverState::Failed;
			...
			Event::SelectionClear(event) => {
				if let Some(selection) = context.kind_of(event.selection) {
					...
					let mut data_guard = selection.data.write();
					*data_guard = None;
					// 已接受的 INCR 保留自己的快照，失去 ownership 也必须继续服务。
					let _guard = selection.mutex.lock();
					selection.data_changed.notify_all();
				}
			}
			Event::SelectionRequest(event) => {
				let complete = match context.handle_selection_request(event, &mut sender) {
					Ok(complete) => complete,
					Err(error) => {
						error!("Failed to handle selection request: {error}");
						continue;
					}
				};
				let mut state = context.handover_state.lock();
				if state.active() {
					if sender.has_handover() {
						*state = ManagerHandoverState::Transferring;
						context.handover_cv.notify_all();
					}
					if complete && event.target != context.atoms.TARGETS {
						written = true;
					}
				}
			}
			Event::SelectionNotify(event)
				if event.selection == context.atoms.CLIPBOARD_MANAGER
					&& context.handover_state.lock().active() =>
			{
				// 某些 manager 先通知再请求数据；必须等真正的数据确认，不能在此结束。
				notified = true;
			}
			_ => (),
		}
