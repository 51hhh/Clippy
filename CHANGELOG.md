...
- 新增键盘优先的动作启动器，可从托盘或主窗口 `Ctrl/Cmd+K` 打开并搜索截图、复制文字和翻译文字；
  参数先由后端校验并换取一次性句柄，运行中、取消、空结果和稳定错误均有明确状态。需要可信图片
  来源的 OCR、扫码、保存和 Pin 动作在组合阶段完成前不会显示，避免用伪造图片引用执行。
  （需求：`PX-ACT-01`）
...
- Pin 增加用户主动保存的本机工作区：临时贴图保持一次性，书签保存后才会跨重启恢复。工作区记录
  ...
  窄 Pin 窗口中面板会自适应并滚动。独立的全局工作区历史浏览仍待后续界面阶段。
  （需求：`PX-PIN-01`）
