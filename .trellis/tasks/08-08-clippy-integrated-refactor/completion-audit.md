...
| 5 | 冻结覆盖层、区域/窗口选择、移动和八向缩放 | 部分通过 | CaptureSession、多显示器帧、窗口候选和 React geometry 已实现；`7f9942e` 在动作开始前原子认领会话，失败路径统一关闭覆盖层并恢复源窗口；Rust 混合缩放、竞态/清理及前端八向 geometry 测试通过，仍需 GNOME/KDE 视觉与输入验收。 |
