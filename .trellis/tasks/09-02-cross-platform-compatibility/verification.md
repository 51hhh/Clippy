...
### 2026-09-03 截图性能与大尺寸 Pin 画质复核

- `render_capture` 不再为小选区调整、效果处理并二次编解码整张 4K 冻结帧；它只保留输出视口与
  blur/mosaic/magnifier 所需邻域。跨边界 blur、mosaic、magnifier、spotlight 与完整渲染后裁剪
  逐像素一致，矢量输出由固定 RGBA digest 约束。release 探针中 3840×2160 输入、640×480
  输出的实际工作集为 688×528，耗时 88.7 ms。
- 大尺寸 Pin 不再因旧 700 万像素阈值静默跳过屏显补偿。3840×2160 屏在 device scale 1.5 /
  WebKit buffer scale 2 下得到 3840×2160 panel 与 5120×2880 buffer；生产实现改为 Q7 定点、
  按需 Lanczos 行缓存和逐行双线性反投影。相同高频 fixture 上旧全图 f32 路径为 24.411 dB，
  新路径为 24.595 dB；断言禁止新路径低于旧路径。
- release 性能探针：1600×1200 为 178.7 ms、3413×1920 为 636.4 ms、5120×2880 为
  1.303 s。补偿在窗口首帧之后的全局串行后台任务运行；64 MiB 是单个 RGBA 平面的明确上限，
  不是总 RSS。关闭后仍在排队的任务会在 PNG 解码前取消，已开始的旧任务不能向同 label 新窗口
  存图或发事件。
- 当前完整 `./scripts/ci-local.sh` 为 12 通过、0 失败、1 个按开关跳过：Rust 439 项通过、
  前端 836 项通过，DOM/Xvfb、Canvas 像素、主窗口布局和生产构建全部通过。
- 当前 Ubuntu GNOME Wayland 双屏真实链路连续两次从 D-Bus 调用到覆盖层就绪：冻结帧分别为
  167/158 ms，较慢的 4K 覆盖层总耗时分别为 927/847 ms；不再出现此前每块屏等待 350 ms
  PipeWire 首帧超时并逐屏 PNG 兜底的 1.3–1.7 s 路径。
- 复制与保存数据流未改：屏显补偿只进入 Pin WebView；普通图片仍复制/保存原 PNG，可编辑工程仍
  复制/保存权威 composite，并保留绑定原图与操作层的工程元数据。
